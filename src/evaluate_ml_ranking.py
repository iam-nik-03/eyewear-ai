...
MODEL_PATH = PROJECT_ROOT / "models/recommendation/hybrid_model.joblib"
OUTPUT_PATH = PROJECT_ROOT / "data/processed/recommendations/ml_ranking_evaluation.csv"
...
print()
print("=" * 64)
print("AURA — TRAINED ML RANKING EVALUATION")
print("=" * 64)

# Normalized embedding lookup.
embedding_lookup = {}
for index, row in metadata.iterrows():
    frame_id = str(row["frame_id"])
    vector = embeddings[index].astype(float)
...
def metadata_score(row):
    return (
        0.50 * float(row["face_compatibility"])
        + 0.25 * float(row["style_match"])
        + 0.15 * float(row["color_match"])
        + 0.10 * float(row["frame_type_match"])
    )
...
def metrics(relevance, k):
    ...
    relevant_total = int(np.sum(relevance))
    relevant_top = int(np.sum(top_k))

    precision = relevant_top / actual_k
    recall = relevant_top / relevant_total if relevant_total else 0.0
    hit_rate = int(relevant_top > 0)
...
for user_id in df["user_id"].unique():
    ...
    # every positive interaction becomes a separate test case.
    for held_out_frame in liked_frames:
        ...
        # User visual profile is built ONLY from training/history likes.
        ...
        for _, row in user_df.iterrows():
            ...
            candidate_rows.append({
                "frame_id": frame_id,
                "liked": int(row["liked"]),
                "face_compatibility": float(row["face_compatibility"]),
                "visual_preference_score": visual_score,
                "style_match": float(row["style_match"]),
                "color_match": float(row["color_match"]),
                "frame_type_match": float(row["frame_type_match"]),
        ...
        candidates["ml_probability"] = ml_probability

        rankings = {
            "Metadata": candidates.sort_values(
                "metadata_score",
                ascending=False
            ),
            "Visual": candidates.sort_values(
                "visual_preference_score",
                ascending=False
            ),
            "Weighted_Hybrid": candidates.sort_values(
                "weighted_hybrid_score",
                ascending=False
            ),
            "ML_Hybrid": candidates.sort_values(
                "ml_probability",
                ascending=False
            ),
        }
        ...
        for model_name, ranked in rankings.items():
            ...
            for k in [1, 3, 5]:
                ...
                precision, recall, hit_rate, ndcg = metrics(
                    relevance,
                    k
                )
